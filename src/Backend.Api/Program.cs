using Backend.Api.Apis; using Backend.Api.Auth; using Backend.Persistence.Data; using Microsoft.EntityFrameworkCore; using Microsoft.AspNetCore.HttpOverrides; using System.Net; using OpenTelemetry.Metrics; using OpenTelemetry.Trace; using Serilog; using Swashbuckle.AspNetCore.SwaggerGen; using AuthOptions = Backend.Api.Auth.SessionOptions;
...
builder.Services.AddOptions<AuthOptions>()
    ...
    .Validate(options => AuthOptions.IsStrongSigningKey(options.SigningKey), "Authentication:SigningKey must be base64-encoded 32 bytes (256 bits); this format check does not prove entropy, which must be guaranteed by secret generation")
...
app.UseForwardedHeaders(new ForwardedHeadersOptions
{
    ...
    // In production, replace this with the address of the trusted ingress/proxy.
    KnownProxies = { IPAddress.Loopback }
});
using (var scope=app.Services.CreateScope()) { scope.ServiceProvider.GetRequiredService<AppDbContext>().Database.Migrate(); } app.UseSerilogRequestLogging(); app.UseMiddleware<SessionMiddleware>(); app.UseSwagger(); app.UseSwaggerUI(); app.MapAuthEndpoints(); app.MapWorkspaceEndpoints(); app.Run();
