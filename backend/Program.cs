...
app.MapPut("/api/workspaces/{id:guid}/assessment", async (HttpContext http, Guid id, AssessmentInput input, DashboardDb db) => { var w = await db.Workspaces.FindAsync(id); if (w == null) return Results.NotFound(); if (!SecurityRules.HasTenantAccess(TenantContext.Get(http), w.TenantId)) return Results.StatusCode(StatusCodes.Status403Forbidden); var c = await db.Clients.FirstOrDefaultAsync(x => x.Name.ToLower() == input.ClientName.Trim().ToLower()); if (c == null) { c = new Client { Name = input.ClientName.Trim() }; db.Clients.Add(c); await db.SaveChangesAsync(); } var a = await db.Assessments.FirstOrDefaultAsync(x => x.WorkspaceId == id); if (a == null) { a = new Assessment { WorkspaceId = id }; db.Assessments.Add(a); } a.ClientId = c.Id; a.Content = string.IsNullOrWhiteSpace(input.Content) ? Domain.AssessmentTemplate : input.Content; a.Status = input.Completed ? AssessmentStatus.Completed : AssessmentStatus.InProgress; a.UpdatedAt = DateTimeOffset.UtcNow; if (input.Completed) w.ClientId = c.Id; await db.SaveChangesAsync(); return Results.Ok(a); });
...
app.MapPost("/api/webhooks/phase", async (HttpContext http, PhaseEvent e, DashboardDb db) =>
{
    var p = await db.Pipelines.AsNoTracking().SingleOrDefaultAsync(x => x.ExternalRef == e.ExternalRef);
    if (p == null) return Results.NotFound();
    var workspaceTenant = await db.Workspaces.Where(x => x.Id == p.WorkspaceId).Select(x => x.TenantId).SingleOrDefaultAsync();
    if (workspaceTenant == null || !SecurityRules.HasTenantAccess(TenantContext.Get(http), workspaceTenant)) return Results.StatusCode(StatusCodes.Status403Forbidden);
    if (p.CurrentPhase == e.TargetPhase) return Results.Ok(new { changed = false });

    await using var transaction = await db.Database.BeginTransactionAsync();
    var updated = await db.Pipelines.Where(x => x.Id == p.Id && x.CurrentPhase == p.CurrentPhase)
        .ExecuteUpdateAsync(setters => setters
            .SetProperty(x => x.CurrentPhase, e.TargetPhase)
            .SetProperty(x => x.GateStatus, e.GateStatus));
    if (updated == 0) return Results.Conflict(new { message = "A pipeline foi alterada antes do processamento do evento." });
    db.Transitions.Add(new PhaseTransition { PipelineInstanceId = p.Id, Phase = e.TargetPhase, SourceEvent = e.SourceEvent });
