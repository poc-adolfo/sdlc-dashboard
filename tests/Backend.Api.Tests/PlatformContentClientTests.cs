...
public sealed class PlatformContentClientTests
{
    ...
    [Fact]
    public async Task ListLabeledIssuesAsync_ReturnsNumberAndBodyAndExcludesPullRequests()
    {
        var handler = new RoutingFakeHandler();
        handler.On(r => r.Method == HttpMethod.Get && r.RequestUri!.AbsolutePath == "/repos/acme/platform/issues", _ =>
            new HttpResponseMessage(HttpStatusCode.OK)
            {
                Content = JsonContent.Create(new object[]
                {
                    new { number = 7, body = "US body" },
                    new { number = 8, body = (string?)null },
                    new { number = 9, body = "actually a PR", pull_request = new { url = "https://api.github.com/repos/acme/platform/pulls/9" } }
                })
            });

        var result = await BuildClient(handler).ListLabeledIssuesAsync(GitHubWorkspace(), "sdlc-pipeline", CancellationToken.None);

        Assert.NotNull(result);
        Assert.Equal(2, result!.Count);
        Assert.Contains(result, x => x.Number == "7" && x.Body == "US body");
        Assert.Contains(result, x => x.Number == "8" && x.Body == ""); // missing/null body normalizes to "", never throws
    }

    [Fact]
    public async Task ListLabeledIssuesAsync_QueriesByLabelAndAllStatesAndSendsBearerAuth()
    {
        var handler = new RoutingFakeHandler();
        handler.On(r => r.Method == HttpMethod.Get && r.RequestUri!.AbsolutePath == "/repos/acme/platform/issues", _ =>
            new HttpResponseMessage(HttpStatusCode.OK) { Content = JsonContent.Create(Array.Empty<object>()) });

        await BuildClient(handler).ListLabeledIssuesAsync(GitHubWorkspace(), "sdlc-pipeline", CancellationToken.None);

        var call = Assert.Single(handler.Captured);
        Assert.Contains("labels=sdlc-pipeline", call.Uri.Query);
        Assert.Contains("state=all", call.Uri.Query);
        Assert.Equal("Bearer", call.Headers.Authorization?.Scheme);
        Assert.Equal("test-token", call.Headers.Authorization?.Parameter);
    }

    [Fact]
    public async Task ListLabeledIssuesAsync_NonSuccessStatusReturnsNull()
    {
        var handler = new RoutingFakeHandler();
        handler.On(_ => true, _ => new HttpResponseMessage(HttpStatusCode.InternalServerError));
    ...
    [Fact]
    public async Task ListLabeledIssuesAsync_MalformedJsonReturnsNull()
    {
        var handler = new RoutingFakeHandler();
    ...
    [Fact]
    public async Task ListLabeledIssuesAsync_UnexpectedShapeReturnsNullInsteadOfThrowing()
    {
        // GitHub always returns an array for this endpoint, but guard the case it doesn't (e.g. an
        // error object) rather than letting EnumerateArray() throw.
        var handler = new RoutingFakeHandler();
        handler.On(_ => true, _ => new HttpResponseMessage(HttpStatusCode.OK) { Content = JsonContent.Create(new { message = "not an array" }) });
    ...
    [Fact]
    public async Task ListLabeledIssuesAsync_AzureDevOpsReturnsNullWithoutAnHttpCall()
    {
        var handler = new RoutingFakeHandler();
        handler.On(_ => true, _ => throw new InvalidOperationException("must not call the platform for Azure DevOps"));

        var result = await BuildClient(handler).ListLabeledIssuesAsync(AzureDevOpsWorkspace(), "sdlc-pipeline", CancellationToken.None);
